---
title: You may not need if-else statement😎
...
In this example, we created two if statements. One with the traditional method and the other with the && operator, to check if `(a>b)` is truthy. As we can see, both cases are logged on the same console.
